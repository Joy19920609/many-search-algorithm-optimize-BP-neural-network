...
plotperform(per2)
figure
plot(test_simu(end-99:end))
hold on
plot(T_test(end-99:end))
legend('实际输出','期望输出')
title('优化前')
%% 粒子群优化bp


[bestchrom,trace]=psoforbp(inputnum,hiddennum,outputnum,Pn_train,Tn_train,Pn_test,Tn_test);%粒子群算法
x=bestchrom;


%% 优化后结果分析
 figure
plot(trace(:,1),'b--');hold on
plot(trace(:,2),'*-');
legend('validation','Training')
title('适应度曲线图')
xlabel('进化代数');ylabel('均方差');
